...
?>
   ...
   <textarea form="texts" name="<?=$i?>" rows="2" cols=60><?php echo $row["Main"]; ?></textarea>
   <?php $mapperMain[$i]=$row["Main"];
   $i++;
   ?>
...
<input type="submit" name="Edit" form="texts"/>
    ...
    <?php
            if((isset($_POST['Edit']))){
                ...
                $j=$i;
                for(;$j>=0;$j--){
                    if(array_key_exists($j,$mapperMain)){
                        if(array_key_exists(strval($j),$_POST)){
                          
                        if($_POST[strval($j)]!=$mapperMain[$j]){
                          $new= $_POST[strval($j)];
                          $old=$mapperMain[$j];
